...
import RoomOfBookingModal from "./roomBOokingModal";
...
const RoomsCard: FC<RoomsCardProps> = ({
  ...
  return (
    <>
      <CarouselItem className="mb-5 h-auto rounded-lg bg-white shadow-lg transition-shadow duration-300 hover:shadow-[0_4px_20px_rgba(0,0,0,0.25)] md:basis-1/3 lg:basis-1/4">
        ...
        <div className="space-y-1 px-2 pb-4">
          ...
          <button
            ...
            className="mt-4 w-full cursor-pointer rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700"
          >
            Book now for {price} $
          </button>
        </div>
      ...
      {isOpen && (
        <RoomOfBookingModal onClose={() => setIsOpen(false)} room_id={id} />
      )}
    </>
  );
};
